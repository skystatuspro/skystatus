// src/hooks/useUserDataV2.ts
// React Query-based replacement for useUserData
// DROP-IN REPLACEMENT: Same interface, better implementation
//
// Key differences from useUserData:
// 1. No debounced auto-save - mutations save immediately
// 2. No stale closures - React Query cache is always fresh
// 3. Optimistic updates with automatic rollback on error
// 4. Server is source of truth, not local state
...
export type { QualificationSettings } from './useUserData';
...
export function useUserDataV2(): UseUserDataReturn {
  const { user } = useAuth();
  ...
  const setBaseXpData = useCallback((_data: XPRecord[]) => {
    ...
    console.log('[useUserDataV2] setBaseXpData called - this is a no-op in V2');
  }, []);
  ...
  const handlePdfImport = useCallback((
    ...
    console.log('[useUserDataV2] handlePdfImport called', {
  ...
  const handleStartOver = useCallback(async () => {
    console.log('[useUserDataV2] handleStartOver');
  ...
  const markDataChanged = useCallback(() => {
    ...
    console.log('[useUserDataV2] markDataChanged called - no-op in V2');
  }, []);
  ...
  const forceSave = useCallback(async () => {
    ...
    console.log('[useUserDataV2] forceSave called - no-op in V2 (changes save immediately)');
  }, []);
...
export { useUserDataV2 as useUserData };
