...
const ViewModeToggle: React.FC = () => {
  ...
  if (!viewModeContext) return null;
  
  const { viewMode, setViewMode } = viewModeContext;
...
export const Layout: React.FC<LayoutProps> = ({
  ...
  // Use optional hook - fallback to full mode if context not available
  const viewModeContext = useViewModeOptional();
        ...
        <div className="mt-auto pt-6 flex-shrink-0 space-y-3">
          ...
          <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm space-y-2">
            <div>
                ...
                Flying Blue v2.2 <span className="ml-1 px-1.5 py-0.5 text-[8px] bg-amber-500/20 text-amber-400 rounded font-bold">BETA</span>
                </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-white">Beta v2.2</span>
                  <span className="relative flex h-2 w-2">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                  </span>
